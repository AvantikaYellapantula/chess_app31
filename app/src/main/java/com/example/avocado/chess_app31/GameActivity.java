...
//		gamelist= GameList.getData(); need to fix this later!!!

            Scanner input= new Scanner(System.in);//take this out later
            ...
            while (noMistake == false) {
              ...
              //  strInput = input.nextLine(); this is for console entering*********
                ...
                while(strInput.length()<5){
                    //this means that we need the handle input method to be done
                }

                noMistake = gameView.acceptArg(strInput);
                strInput="";//took an input from board now reset it for next input
